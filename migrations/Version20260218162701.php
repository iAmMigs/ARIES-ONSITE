 ...
 */
final class Version20260218162701 extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Adds Database Triggers for comprehensive Audit Trails with NULL remarks';
    }

    public function up(Schema $schema): void
    {
        // --- 1. AUDIT TRIGGERS FOR BED_APPLICANTS ---
        // Note: 'created_at' must exist in audit_bed_applicants table
    ...
    private function createTriggers(string $sourceTable, string $auditTable, array $columns, array $mappingOverrides = []): void
    {
        ...
        $this->addSql("
            ...
            BEGIN
                ...
                VALUES ($newValuesStr, 'INSERT', $userSql, NOW(), $hostSql, NULL);
            END
        ");
        ...
        $this->addSql("
            ...
            BEGIN
                INSERT INTO `$auditTable` ($targetColsStr, $auditCols)
                VALUES ($newValuesStr, 'UPDATE', $userSql, NOW(), $hostSql, NULL);
            END
        ");
        ...
        $this->addSql("
            ...
            BEGIN
                INSERT INTO `$auditTable` ($targetColsStr, $auditCols)
                VALUES ($oldValuesStr, 'DELETE', $userSql, NOW(), $hostSql, NULL);
            END
        ");
    }
}